...
import { z } from "zod";
import { fetchProjectTasks, updateProject, deleteProject, updateProjectMembers } from "../../store/slices/projectSlice.js";
import { fetchWorkspaceDetails } from "../../store/slices/workspaceSlice.js";
...
import { Plus, Loader2, CalendarDays, AlertCircle, Clock, CheckCircle, BookOpen, Settings, Trash2, Pencil, Users } from "lucide-react";
...
function TaskCard({ task, onClick, projectId, canManageTasks }) {
  const dispatch = useDispatch();
...
function TaskColumn({ status, tasks, onTaskClick, projectId, canManageTasks }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="space-y-2">
        {tasks.map((task) => <TaskCard key={task._id} task={task} onClick={onTaskClick} projectId={projectId} canManageTasks={canManageTasks} />)}
...
function EditProjectDialog({ open, onOpenChange, project, workspaceId, workspaceMembers, isManager }) {
  const dispatch = useDispatch();
  ...
  const [selectedMembers, setSelectedMembers] = useState(
    (project?.members || []).map((m) => ({ user: m.user._id || m.user, role: m.role }))
  );

  useEffect(() => {
    if (open) {
      setSelectedMembers((project?.members || []).map((m) => ({ user: m.user._id || m.user, role: m.role })));
    }
  }, [open, project]);

  const toggleMember = (userId) =>
    setSelectedMembers((prev) =>
      prev.find((m) => m.user === userId)
        ? prev.filter((m) => m.user !== userId)
        : [...prev, { user: userId, role: "contributor" }]
    );
  const updateMemberRole = (userId, role) =>
    setSelectedMembers((prev) => prev.map((m) => (m.user === userId ? { ...m, role } : m)));
  ...
  const handleSaveMembers = async () => {
    const result = await dispatch(updateProjectMembers({ projectId: project._id, members: selectedMembers }));
    if (updateProjectMembers.fulfilled.match(result)) {
      toast.success("Members updated!");
      dispatch(fetchProjectTasks(project._id));
    } else { toast.error(result.payload || "Failed to update members"); }
  };
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="w-full">
            {isManager && <TabsTrigger value="edit" className="flex-1"><Pencil className="h-3.5 w-3.5 mr-1.5" />Edit</TabsTrigger>}
            {isManager && <TabsTrigger value="members" className="flex-1"><Users className="h-3.5 w-3.5 mr-1.5" />Members</TabsTrigger>}
            {isManager && <TabsTrigger value="danger" className="flex-1 text-destructive"><Trash2 className="h-3.5 w-3.5 mr-1.5" />Delete</TabsTrigger>}
          </TabsList>
          ...
          <TabsContent value="members" className="mt-4 space-y-3">
            <p className="text-sm text-muted-foreground">Add or remove workspace members from this project and set their roles.</p>
            <div className="border rounded-md divide-y max-h-72 overflow-y-auto">
              {(workspaceMembers || []).map(({ user: u }) => {
                const isSel = !!selectedMembers.find((m) => m.user === u._id);
                return (
                  <label key={u._id} className="flex items-center gap-3 p-3 hover:bg-muted/50 cursor-pointer">
                    <input type="checkbox" checked={isSel} onChange={() => toggleMember(u._id)} className="rounded shrink-0" />
                    <Avatar className="h-7 w-7 shrink-0"><AvatarImage src={u.profilePicture} /><AvatarFallback className="text-xs">{u.name?.charAt(0)}</AvatarFallback></Avatar>
                    <span className="text-sm flex-1 truncate">{u.name}</span>
                    {isSel && (
                      <select
                        className="text-xs border rounded px-1.5 py-1 bg-background shrink-0"
                        value={selectedMembers.find((m) => m.user === u._id)?.role || "contributor"}
                        onChange={(e) => { e.stopPropagation(); updateMemberRole(u._id, e.target.value); }}
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
              <Button onClick={handleSaveMembers} disabled={loading}>{loading ? <><Loader2 className="h-4 w-4 mr-2 animate-spin" />Saving...</> : "Save Members"}</Button>
            </DialogFooter>
          </TabsContent>
          )}
...
export default function ProjectDetails() {
  ...
  const navigate = useNavigate();
  const { currentProject, tasks, loading, isWorkspaceOwner } = useSelector((s) => s.project);
  ...
  useEffect(() => {
    ...
    if (workspaceId) dispatch(fetchWorkspaceDetails(workspaceId));
  }, [projectId, workspaceId, dispatch]);
  ...
  const currentProjectMember = currentProject.members?.find((m) => (m.user._id || m.user) === user?._id);
  const projectRole = currentProjectMember?.role;
  const isManager = projectRole === "manager" || isWorkspaceOwner;

  // Owner sees ALL tasks; manager sees tasks they created + assigned; members see only assigned
  const visibleTasks = isWorkspaceOwner
    ? tasks
    : isManager
    ? tasks.filter((t) => (t.createdBy?._id || t.createdBy) === user?._id || t.assignees?.some((a) => (a._id || a) === user?._id))
    : tasks.filter((t) => t.assignees?.some((a) => (a._id || a) === user?._id));

  const progress = getProjectProgress(tasks);
  const byStatus = STATUSES.reduce((acc, s) => ({ ...acc, [s]: visibleTasks.filter((t) => t.status === s) }), {});
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          ...
          {isManager && <Button onClick={() => setCreateOpen(true)}><Plus className="h-4 w-4 mr-2" />Add Task</Button>}
          {isManager && <Button variant="outline" size="icon" onClick={() => setEditOpen(true)} title="Project Settings"><Settings className="h-4 w-4" /></Button>}
        </div>
      </div>
      ...
      {!isManager && !isWorkspaceOwner && (
        <p className="text-xs text-muted-foreground bg-muted/50 rounded-md px-3 py-2">
          Showing tasks assigned to you. Managers can see all tasks they created.
        </p>
      )}
      ...
      <Tabs defaultValue="all">
        ...
        <TabsContent value="all" className="mt-6">
          <div className="grid md:grid-cols-4 gap-6">
            {STATUSES.map((s) => <TaskColumn key={s} status={s} tasks={byStatus[s]} onTaskClick={handleTaskClick} projectId={projectId} canManageTasks={isManager} />)}
          </div>
        </TabsContent>
        {STATUSES.map((s) => (
          <TabsContent key={s} value={s} className="mt-6">
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {byStatus[s].map((task) => <TaskCard key={task._id} task={task} onClick={handleTaskClick} projectId={projectId} canManageTasks={isManager} />)}
