...
import { updateProfile, uploadAvatar, changePassword, enable2FA, verify2FA, disable2FA } from "../../store/slices/authSlice.js";
...
export default function Profile() {
  ...
  const handleFileChange = async (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      const result = await dispatch(uploadAvatar(base64Image));
      setAvatarUploading(false);
      if (uploadAvatar.fulfilled.match(result)) {
        setAvatarPreview(result.payload.profilePicture);
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()}
                  ...
                  className="absolute bottom-0 right-0 h-7 w-7 rounded-full bg-primary text-primary-foreground flex items-center justify-center hover:bg-primary/90 transition-colors shadow disabled:opacity-60">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {avatarUploading ? "Uploading to Cloudinary..." : "JPG, PNG or GIF · Max 5MB · Click camera to change"}
                </p>
              </div>
            </div>
