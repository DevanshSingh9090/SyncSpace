                  ...
                  <label key={user._id} className="flex items-center gap-2 p-1.5 rounded hover:bg-muted cursor-pointer">
                    ...
                    {selectedMembers.find((m) => m.user === user._id) && (
                      <select className="text-xs border rounded px-1 py-0.5 bg-background" value={selectedMembers.find((m) => m.user === user._id)?.role || "contributor"} onChange={(e) => { e.stopPropagation(); updateMemberRole(user._id, e.target.value); }} onClick={(e) => e.stopPropagation()}>
                        <option value="manager">Manager</option><option value="contributor">Contributor</option>
                      </select>
                    )}
                  </label>
                ))}
...
function WorkspaceSettingsDialog({ open, onOpenChange, workspace }) {
  ...
  const isOwner = workspace?.members?.find((m) => m.user._id?.toString() === user?._id?.toString() && m.role === "owner");
  const isAdmin = workspace?.members?.find((m) => m.user._id?.toString() === user?._id?.toString() && ["owner", "admin"].includes(m.role));
  const eligibleForTransfer = workspace?.members?.filter((m) => m.user._id?.toString() !== user?._id?.toString()) || [];
...
export default function WorkspaceDetails() {
  ...
  const currentUserRole = currentWorkspace.members?.find((m) => m.user._id?.toString() === user?._id?.toString())?.role;
  const canManage = ["owner", "admin"].includes(currentUserRole);
  // isOwner: check both member role AND the workspace.owner field as a reliable fallback
  const isOwner = currentUserRole === "owner"
    || currentWorkspace.owner?._id?.toString() === user?._id?.toString()
    || currentWorkspace.owner?.toString() === user?._id?.toString();
  // Workspace owner and admins can create projects
  const canCreateProject = isOwner || currentUserRole === "admin";
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {canCreateProject && <Button size="sm" onClick={() => setCreateProjectOpen(true)}><Plus className="h-4 w-4 mr-2" />New Project</Button>}
      ...
      <Tabs defaultValue="projects">
        ...
        <TabsContent value="projects" className="mt-6">
          {!currentProjects?.length ? (
            <NoDataFound title="No projects yet" description={canCreateProject ? "Create your first project in this workspace" : "No projects have been assigned to you yet"} buttonText={canCreateProject ? "Create Project" : undefined} buttonAction={canCreateProject ? () => setCreateProjectOpen(true) : undefined} />
          ) : (
                ...
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge
                      variant="outline"
                      className={cn(
                        "text-xs capitalize shrink-0 font-semibold border",
                        role === "owner"
