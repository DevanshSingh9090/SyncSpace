...
export const fetchArchivedProjects = createAsyncThunk("project/fetchArchived", async (workspaceId, { rejectWithValue }) => {
  try { const res = await api.get(`/projects/workspaces/${workspaceId}/archived`); return res.data; }
  catch (err) { return rejectWithValue(err.response?.data?.message || "Failed to fetch archived projects"); }
});
...
const projectSlice = createSlice({
  ...
  reducers: {
    ...
    clearCurrentProject(state) { state.currentProject = null; state.tasks = []; },
  },
  extraReducers: (builder) => {
    ...
    builder
      .addCase(fetchArchivedProjects.pending, (state) => { state.loading = true; state.error = null; })
      .addCase(fetchArchivedProjects.fulfilled, (state, action) => { state.loading = false; state.archivedProjects = action.payload; })
      .addCase(fetchArchivedProjects.rejected, (state, action) => { state.loading = false; state.error = action.payload; });
  },
});
