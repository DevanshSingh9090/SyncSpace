...
const getProjectTasks = async (req, res) => {
  try {
    ...
    // Check if user is a workspace owner — owners bypass project membership
    ...
    res.status(200).json({ project, tasks, isWorkspaceOwner });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Internal server error" });
  }
...
const updateProjectMembers = async (req, res) => {
  try {
    ...
    await project.save();
    const updated = await Project.findById(projectId).populate("members.user", "name profilePicture email");
    res.status(200).json({ message: "Members updated successfully", project: updated });
  } catch (error) {
    console.log(error);
    return res.status(500).json({ message: "Internal server error" });
  }
...
export { createProject, getProjectDetails, getProjectTasks, updateProject, deleteProject, updateProjectMembers };
